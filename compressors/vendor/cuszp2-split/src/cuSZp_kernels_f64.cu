...
__global__ void cuSZp_compress_kernel_outlier_f64(const double* const __restrict__ oriData, 
                                                  ...
                                                  const double eb, const size_t nbEle)
{
    ...
    // FP64 quantization (double-precision multiply/add/convert) is expensive enough
    // on this GPU that redoing it in phase 2 (the recompute-instead-of-store trick
    // used for f32) measured as a net loss here -- keep the full per-thread buffer
    // and only pay the (now fenced-once, multi-warp-scanned) memory cost instead.
                                                ...
                                                const double eb, const size_t nbEle)
{
    ...
    // FP64 quantization is expensive enough on this GPU that redoing it in phase 2
    // (the recompute-instead-of-store trick used for f32) measured as a net loss
    // here -- keep the full per-thread buffer instead.
    int absQuant[cmp_chunk];
    unsigned int sign_flag[block_num];
    int sign_ofs;
    int fixed_rate[block_num];
