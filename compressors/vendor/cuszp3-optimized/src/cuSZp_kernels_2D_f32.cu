#include "cuSZp_kernels_2D_f32.h"
                                                        ...
                                                        uint blockNum, const uint3 dims, const float eb)
{
    ...
    for(uint j=0; j<block_per_thread; j++)
    {
        ...
        if(block_idx < blockNum)
        {
            ...
            for(uint i=0; i<8; i++)
            {
                uint data_idx_y = block_idx_y * 8 + i;
                ...
                if(data_idx_y < dims.y)
                {
                    ...
                    if(data_idx_x + 4 < dims.x)
                    {
                        ...
                        sign_ofs = (i * 8 + 7) % 32;
                        sign_flag[2*j+i/4] |= (currQuant < 0) << (31 - sign_ofs);
                        absQuant[quant_chunk_idx+7] = abs(currQuant);
                        maxQuant = maxQuant > absQuant[quant_chunk_idx+7] ? maxQuant : absQuant[quant_chunk_idx+7];
                    }
                    else
                    {
                        absQuant[quant_chunk_idx+4] = 0;
                        absQuant[quant_chunk_idx+5] = 0;
                        absQuant[quant_chunk_idx+6] = 0;
                        absQuant[quant_chunk_idx+7] = 0;
                    }
                }
                else
                {
                    for(uint k=0; k<8; k++) absQuant[quant_chunk_idx+k] = 0;
                }
            }
    ...
    for(uint j=0; j<block_per_thread; j++)
    {
        ...
        if(block_idx < blockNum && fixed_rate[j])
        {
            ...
            for(uint i=0; i<8; i++)
            {
                ...
                {
                    sign_ofs = (i * 8) % 32;
                    currQuant = sign_flag[i/4] & (1 << (31 - sign_ofs)) ? absQuant[i*8] * -1 : absQuant[i*8];
                    dec_buffer.x = currQuant * eb * 2;

                    sign_ofs = (i * 8 + 1) % 32;
                    currQuant = sign_flag[i/4] & (1 << (31 - sign_ofs)) ? absQuant[i*8+1] * -1 : absQuant[i*8+1];
                    dec_buffer.y = currQuant * eb * 2;

                    sign_ofs = (i * 8 + 2) % 32;
                    currQuant = sign_flag[i/4] & (1 << (31 - sign_ofs)) ? absQuant[i*8+2] * -1 : absQuant[i*8+2];
                    ...
                    if(data_idx_x + 4 < dims.x)
                    {
                        ...
                        sign_ofs = (i * 8 + 6) % 32;
                        currQuant = sign_flag[i/4] & (1 << (31 - sign_ofs)) ? absQuant[i*8+6] * -1 : absQuant[i*8+6];
                        dec_buffer.z = currQuant * eb * 2;

                        sign_ofs = (i * 8 + 7) % 32;
                        currQuant = sign_flag[i/4] & (1 << (31 - sign_ofs)) ? absQuant[i*8+7] * -1 : absQuant[i*8+7];
                        dec_buffer.w = currQuant * eb * 2;
            ...
            for(uint i=0; i<8; i++)
            {
                ...
                if(data_idx_y < dims.y)
                {
                    ...
                    if(data_idx_x + 4 < dims.x)
                    {
                        ...
                        sign_ofs = (i * 8 + 7) % 32;
                        sign_flag[2*j+i/4] |= (lorenQuant < 0) << (31 - sign_ofs);
                        absQuant[quant_chunk_idx+7] = abs(lorenQuant);
                        maxQuant = maxQuant > absQuant[quant_chunk_idx+7] ? maxQuant : absQuant[quant_chunk_idx+7];
                    }
                    else
                    {
    ...
    for(uint j=0; j<block_per_thread; j++)
    {
        ...
        if(block_idx < blockNum && fixed_rate[j])
        {
            ...
            for(uint i=0; i<8; i++)
            {
                ...
                {
                    sign_ofs = (i * 8) % 32;
                    lorenQuant = sign_flag[i/4] & (1 << (31 - sign_ofs)) ? absQuant[i*8] * -1 : absQuant[i*8];
                    currQuant = lorenQuant + prevQuant_y;
                    prevQuant_y = currQuant;
                    prevQuant = currQuant;
                    dec_buffer.x = currQuant * eb * 2;

                    sign_ofs = (i * 8 + 1) % 32;
                    lorenQuant = sign_flag[i/4] & (1 << (31 - sign_ofs)) ? absQuant[i*8+1] * -1 : absQuant[i*8+1];
                    currQuant = lorenQuant + prevQuant;
                    prevQuant = currQuant;
                    dec_buffer.y = currQuant * eb * 2;

                    sign_ofs = (i * 8 + 2) % 32;
                    lorenQuant = sign_flag[i/4] & (1 << (31 - sign_ofs)) ? absQuant[i*8+2] * -1 : absQuant[i*8+2];
                    currQuant = lorenQuant + prevQuant;
                    prevQuant = currQuant;
                    dec_buffer.z = currQuant * eb * 2;

                    sign_ofs = (i * 8 + 3) % 32;
                    lorenQuant = sign_flag[i/4] & (1 << (31 - sign_ofs)) ? absQuant[i*8+3] * -1 : absQuant[i*8+3];
                    currQuant = lorenQuant + prevQuant;
                    prevQuant = currQuant;
                    dec_buffer.w = currQuant * eb * 2;
                    ...
                    {
                        sign_ofs = (i * 8 + 4) % 32;
                        lorenQuant = sign_flag[i/4] & (1 << (31 - sign_ofs)) ? absQuant[i*8+4] * -1 : absQuant[i*8+4];
                        currQuant = lorenQuant + prevQuant;
                        prevQuant = currQuant;
                        dec_buffer.x = currQuant * eb * 2;
                        ...
                        sign_ofs = (i * 8 + 6) % 32;
                        lorenQuant = sign_flag[i/4] & (1 << (31 - sign_ofs)) ? absQuant[i*8+6] * -1 : absQuant[i*8+6];
                        currQuant = lorenQuant + prevQuant;
                        prevQuant = currQuant;
                        dec_buffer.z = currQuant * eb * 2;

                        sign_ofs = (i * 8 + 7) % 32;
                        lorenQuant = sign_flag[i/4] & (1 << (31 - sign_ofs)) ? absQuant[i*8+7] * -1 : absQuant[i*8+7];
                        currQuant = lorenQuant + prevQuant;
                        prevQuant = currQuant;
                        dec_buffer.w = currQuant * eb * 2;
            ...
            for(uint i=0; i<8; i++)
            {
                ...
                if(data_idx_y < dims.y)
                {
                    ...
                    if(data_idx_x + 4 < dims.x)
                    {
                        ...
                        absQuant[quant_chunk_idx+7] = abs(lorenQuant);
                        maxQuant = maxQuant > absQuant[quant_chunk_idx+7] ? maxQuant : absQuant[quant_chunk_idx+7];
                        maxQuan2 = maxQuan2 > absQuant[quant_chunk_idx+7] ? maxQuan2 : absQuant[quant_chunk_idx+7];
                    }
                    else
                    {
                        absQuant[quant_chunk_idx+4] = 0;
