...
| D42 | **FZGM's automatic-fusion feature is now "Pipeline Specialization"; benchkit tracks `FZ_SPECIALIZE` in execution identity and the CLI report's `specialization` block, keeping the legacy `fusion` names.** (2026-09) FZGM renamed the finalize-time optimizer to reflect that it does more than fuse kernels (single-pass decoupled-lookback, NVRTC codegen, a roofline profitability gate, and — new this cycle — *decompress*-side fusion for both the warp-register and chunk-cooperative families, plus specialization-aware buffer coloring that lowers peak pool memory). The public API and env var moved (`FZ_FUSION` → `FZ_SPECIALIZE`, `setFusionPolicy` → `setSpecializationPolicy`, …) with back-compat aliases, and `--report-json` now emits a `specialization` object **and** a byte-identical legacy `fusion` object. Benchkit change is deliberately minimal: the adapter still reads `rep.get("fusion")` (unchanged), and `_FZGM_BEHAVIOR_ENV` gains `FZ_SPECIALIZE` alongside `FZ_FUSION` so an off/auto A/B keyed on either name yields distinct `execution_id`s. New preflight: `specialization_vs_native_smoke.yaml` (native vs FZGM-staged vs FZGM-specialized, each launch `FZ_SPECIALIZE=off` then `auto`) and `specialization_memory_smoke.yaml` (peak-memory ablation: {staged, specialized} × {coloring on, off}, via `*_nocolor.toml` presets carrying `coloring = false`), driver `scripts/run-specialization-smoke.sh`. The refreshed performance-first matrix adds native/FZGM cuSZp3 fixed as an intentionally non-specializing control, keeps `szp_composed` as an FZGM-only SZp-inspired composition, and retains neither compressed nor decompressed cell artifacts; memory reporting is FZGM-only until native adapters expose comparable allocation peaks. | The rename is upstream's, not ours, and the `fusion_*` row columns / `fusion` JSON block are load-bearing for existing baselines and parsers — so the guiding principle was "follow the rename in provenance identity and docs, change nothing a stored session or a report reader depends on". Adding `FZ_SPECIALIZE` to the behavior-env tuple is the one non-cosmetic requirement: without it, `FZ_SPECIALIZE=off` and `FZ_SPECIALIZE=auto` sessions would share an `execution_id` and the resume/merge logic would treat them as the same measurement. Same class as D40's logical-vs-execution identity split. |
| D43 | **cuSZp3 fixed comparisons use the flat native `fixed` kernel for every dataset shape; dimensional matching applies only to predictor-bearing plain/outlier modes. Machine GPU architecture is session provenance, not a hardcoded cross-machine experiment flag.** | Fixed has no spatial predictor, and FZGM's preset operates on flat 64-value AdaptiveBitpack blocks. The 2026-09-03 smoke exposed severe `fixed:2d`/`fixed:3d` pointwise-bound misses; direct flat-mode checks restored the expected bound behavior. The specialization campaign also runs unchanged across sm_80, sm_89, sm_90, and gfx908, so its tool declarations carry source/patch/build-recipe identity while checked-in site environments export `BENCHKIT_GPU_ARCH` (and CUDA's numeric `CUDA_ARCH`) into `software.build_environment`. This avoids falsely labeling an L40S/A100 build as H100 while satisfying H3's complete native-tool provenance requirement. |
| D44 | **The 1-D Lorenzo + AdaptiveBitpack specialization family is parameterized by block size and encoding mode, not by compressor name; the former `szp_composed` row is the `B=128, plain` point. Auto currently performs structural matching against an evidence-gated registry and maximizes launches removed, not predictive per-input profitability.** | The controlled H100 sweep at `B={32,64,96,128}` installed both directions in all 120 Auto cells and preserved every archive/reconstruction. It also exposed a block-size-dependent staged cost and one reliable Auto slowdown, so D42's “roofline profitability gate” wording was too strong. The legacy `no_profitable_implementation` result token means no auto-enabled registry match as well as any future cost rejection and must not be interpreted as a measured prediction without a more precise reason taxonomy. |
| D45 | **Repeated native cuSZ-Hi measurements must start from a fresh CLI context; historical narrow-range HURR/SCALE aborts from the local repeated-timing build are invalid coverage evidence. Native tuning for RQ3 is bounded to the exposed interpolation policy and legal spline radius, rather than an open-ended sweep.** | cuSZ-Hi converts `rel_range` bounds to absolute values in place and also mutates interpolation-tuning state. The original local `--repeat` patch reused that context, so repetition three could overflow the fixed outlier workspace even when a single run was valid. Copying the context per repetition restores the missing coverage. Source audit also shows that defaults already use CR-first interpolation tuning, the maximum legal radius (128), and automatic Huffman chunk sizing; therefore RQ3 tests only RD-first and radius 64 on representative fields unless either changes the X2 frontier decision. |
| D46 | **Peak memory is not a headline efficiency claim; it gets the same "specialization recovers the modularity cost" framing as throughput, and is scoped to roughly one section plus one single-column figure in the paper — not a comparable-scale table per family.** `tools/cuda_mem_probe/` (an `LD_PRELOAD` interposer on the `cudaMalloc` family) plus external NVML polling now give native tools a peak-memory number for the first time, superseding D42's "memory reporting is FZGM-only until native adapters expose comparable allocation peaks." A cross-family, cross-size sweep (cuSZ, cuSZp2/3 outlier, PFPL, FZ-GPU, FSZ, plus the FZGM-only `szp_composed`; 8 fields, 11.5MB-1.1GB, one error bound, max of 3 reps) shows FZGM is **not generally more memory-efficient than native** — native's total footprint wins above roughly a few hundred MB per field, once a per-tool fixed cost (CUDA context, module/JIT loading, and — largest for cuSZp2/3's outlier path specifically — allocator-internal churn invisible to any `cudaMalloc`-family probe) is amortized over a big-enough buffer. Below that crossover, FZGM's persistent pool avoids paying that fixed cost repeatedly and looks far leaner, but that is a small-field artifact, not a scalable efficiency win. **NVML ground truth (the process's true resident memory) is the metric that must be trusted, not the probe alone** — the probe undercounts by exactly this fixed-cost gap (confirmed via a plain-vs-outlier isolation on cuSZp2: the gap is ~1.4GB, constant across all 8 field sizes, and vanishes in plain mode) and would rank tools backwards if used as the primary number. | The plan going into this work was a straightforward "does specialization also reduce memory" ablation; the size sweep instead surfaced that (a) our own self-reported pool peak silently excludes the CLI's input buffer (a known, fixed, exactly-quantified omission — confirmed by an interposer calibration against FZGM's own `getPeakMemoryUsage()`) and (b) at least one native tool's *true* footprint is dominated by memory no self-report or naive probe would ever see. Both facts argue for restraint in how the paper frames this axis: the honest, defensible claim is the same shape as the throughput story (RQ2) — composition/modularity has a real memory cost, and specialization recovers most of it — not a "we beat native on memory" claim the data does not support at realistic field sizes. **Unresolved platform caveat:** the sweep ran on one JetStream2 H100 GPU-passthrough VM (confirmed via `nvidia-smi -q`: `Virtualization Mode: Pass-Through`, but still a KVM guest — IOMMU active with lazy TLB invalidation). This repo already has an independent, unrelated finding that this platform class makes repeated `cudaMalloc`/`cudaFree` calls anomalous (a local cuSZp2 patch, `compressors/cuSZp-V2.0.1/src/cuSZp_entry_f32.cu`, documents "large, highly variable latency... observed on a GPU-passthrough cloud VM" for exactly that call pattern). cuSZp2/3's outlier mode — the source of the largest fixed-overhead numbers here — does its own internal allocate/free churn via CUB, so the *magnitude* of that gap (and therefore the exact crossover point) is not yet cross-machine-validated and should not be quoted as a portable constant; the qualitative direction (native pays a fixed cost FZGM's pool avoids; specialization narrows it) is architecture-driven and more likely to reproduce elsewhere. Full measurement/comparison provenance (tool commits, the FZ-GPU/PFPL shadow-rebuild recipe, field list, per-tool CLI gotchas, and the excluded-cell list): `docs/peak_memory_methodology.md`. Full sweep + crossover chart: `paper_organizer` artifact link in the FZGM memory notes; raw data and driver script are session-local pending a decision on whether this becomes a permanent benchkit adapter feature. |
