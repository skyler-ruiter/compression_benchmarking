...
| `EBLC-SPEC` | Performance-first FZGM Pipeline Specialization vs native, including cuSZp3 fixed | `ANALYSIS; BLOCK-SIZE AND SIZE-CROSSOVER PROBES COMPLETE` | Full off/auto corpus on H100 and L40S; 167/186 fields on A100; H200 smoke; post-`b08c406` H100 gate plus HACC/NYX size probes complete | Use manual block-size sensitivity as supporting evidence and add only a lightweight small-input compression guard. Defer predictive profitability, architecture-aware tuning, and autotuning. Exclude broken historical A100/L40S native `cuszp3_fixed` controls and launch only claim-driven reruns before the final clean H100 campaign |
| `F1-COMPOSED` | Cross-family composed-pipeline frontier | `PAIRED ANALYSIS COMPLETE; WORDING/TUNING GATE OPEN` | H100: HURR, SCALE-LETKF, CESM-2D | Selected `x_lq_pfpl_ans` adds a robust sampled frontier point in 185/305 usable rows, 117/237 rows with complete attempts for ten native variants, and 50/69 rows where all 30 native measurements are usable. The command-line mode audit exposes no obvious missing neutral tuning arm, so use ``tested native configurations'' unless a source-level audit justifies ``tuned.'' Preserve cuSZ-Hi missing attempts and validity failures explicitly. Generated artifact: `../paper_organizer/papers/FZGM/reports/composed_native_frontier.md` |
...
| `3P-CORPUS` | Standalone third-party EBLC on the general scientific corpus | `COMPLETE-DIAGNOSTIC; CURATION REQUIRED` | H100 gate complete; full session contains 3,618 retained attempts for 3,282 logical cells | Do not rerun merely because the measurements are older: deduplicate retries by logical cell, preserve unresolved failures and validity exclusions, and keep CPU, GPU, and mixed-wrapper timing strata separate; rerun only claim-critical missing or timing-unreliable cells |
...
|---|---|---|---|---:|---|
| `F1-COMPOSED` | `20260809-composed-frontier-pin` plus native baselines `20260808-fullcorpus-postfix` and `h100-jetstream2-20260905-spec-vsnative-full-auto` | `js2-h100` | 2026-08-09 | 2,184 composed cells | `COMPLETE-DIAGNOSTIC`: 2,184/2,184 composed runs completed; 2,134 pass the current validity/finite-quality/compression-stability gate across all candidates. The selected `x_lq_pfpl_ans` contributes 305 usable rows and remains on the robust throughput--CR--PSNR sampled native frontier in 185; 117/237 remain when all ten native variants were attempted at all three bounds, and 50/69 remain where all 30 native measurements are usable. The newer native source has precedence and supplies corrected flat cuSZp3-fixed rows; adding it changes none of the selected frontier counts. HURR and SCALE-LETKF are positive; CESM-2D is mixed. The composed source predates H3 row-level dataset hashes and is not a final publication bundle; dimensions, dtype, and original byte count match for all 2,696 usable paired native rows, but the final publication campaign must restore hash-level provenance. Report: `../paper_organizer/papers/FZGM/reports/composed_native_frontier.md`; generator: `scripts/analyze_composed_native_frontier.py`. |
| `F1-COMPOSED` corrected RQ3 | `cuszhi-rq3-coverage-{h100,remainder,cesm}-20260907` and `cuszhi-rq3-tuning-probe-h100-20260907` | `js2-h100` | 2026-09-07 | 624 native cells | `COMPLETE-DIAGNOSTIC`: the apparent HURR/SCALE cuSZ-Hi coverage gap was a mutable-repeat-context failure, not a native abort. Corrected coverage makes all ten native variants attempted for every usable X2 row; X2 is robust in 178/305 (39/39 HURR, 36/36 SCALE-LETKF, 89/230 CESM-2D). The 96-cell, H3-eligible RD-first/radius-64 probe creates no new X2 dominator, closing the bounded tuning gate. The coverage sessions predate the final addition of an explicit `tool_version` declaration and remain diagnostic despite binary/patch artifacts; do not portray them as final publication bundles. Reports: `../paper_organizer/papers/FZGM/reports/{composed_native_frontier,cuszhi_rq3_audit}.md`. |
| `F1-COMPOSED` final paired RQ3 | `fzgm-rq3-x2-paired-final-h100-20260908` | `js2-h100` | 2026-09-08 | 3,432 | `RUNNING`: the matching 11-cell H3/H4 smoke passed after correcting cuSZp3 predictor-mode routing (2-D CESM; 3-D HURR/SCALE). The final session is remeasuring selected X2 (`LorenzoQuant -> Bitshuffle -> RZE -> ANS`) and all ten native frontier configurations over every HURR, SCALE-LETKF, and CESM-2D field at all three `rel_range` bounds. It uses clean FZGM worktree `b90fdfe`, explicit native tool declarations, checksum-locked inputs, locked clocks, and `FZ_SPECIALIZE=off`; run script: `scripts/run-fzgm-rq3-x2-paired-final.sh`. The final figure must read this session only after `benchkit merge` and `benchkit verify` pass. |
| `F1-COMPOSED` RQ3 dataset extension | `fzgm-rq3-x2-paired-extension-h100-20260908` | `js2-h100` | 2026-09-08 | 1,584 | `RUNNING-PREFLIGHT`: shard 0/12 (132 cells) is testing X2 and the same ten native configurations on the remaining compatible float32 3-D families: CESMATM-3D, NYX, QMCPACK-3D, and EXAFEL. This extends the application and size coverage without mixing in f64 data that X2 cannot consume or 1-D data where native cuSZ-Hi is unsupported. The preflight rows are part of the final session; launch shards 1/12 through 11/12 only after shard 0 is clean. Config: `configs/experiments/fzgm_rq3_x2_paired_extension.yaml`; results: `/home/exouser/benchkit-results/fzgm-rq3-x2-paired-extension-h100-20260908`. |
| `EBLC-SPEC` size crossover | `specialization-size-crossover-skyler-h100-{off,auto}-20260907r1` | `js2-h100` | 2026-09-07 04:10 UTC | 80 per arm | `COMPLETE-DIAGNOSTIC`: 80/80 status-ok cells per arm over checksummed HACC/vx prefixes from 2^10 to 2^28, B=32/64/96/128, and plain/outlier modes. Runtime telemetry is present for every Auto row (64 two-pass, 10 thread-independent, 6 single-pass); all 80 pairs preserve compressed size and reconstruction hash. Full FZM hashes differ because an execution-derived internal-buffer size in the header differs; a direct B64/plain/N4096 audit found identical payload CRC and only header/CRC byte differences, but do not generalize that one payload audit to all rows. Auto has 32 reliable compression losses and zero reliable decompression losses, establishing a configuration-dependent small-input crossover. Six Off and seven Auto rows are timing-unreliable, so target or explicitly exclude those before publication use. Generated report: `../paper_organizer/papers/FZGM/reports/specialization_size_crossover.md`; results: `/home/exouser/benchkit-results/specialization-size-crossover-skyler-h100-<policy>-20260907r1`. The interrupted `...-off-20260907` exact-file-size probe contains only expected failures and is not evidence. |
| `EBLC-SPEC` block-size probe | `specialization-blocksize-sweep-skyler-h100-b08-{off,auto}-20260907` | `js2-h100` | 2026-09-07 00:59 UTC | 120 per arm | `COMPLETE-DIAGNOSTIC`: 120/120 status-ok cells per arm from clean `b08c406`; all Auto cells install both directions, with zero compressed-size or reconstruction-hash mismatches. Specialization wins 119/120 reliable compression pairs and every reliable decompression pair. Two Auto HACC/B=64/1e-2 decompression timings are unreliable; eight PSL/1e-4 rows per arm are identical 1.00934x marginal bound misses accepted by the validity tolerance. Generated report: `../paper_organizer/papers/FZGM/reports/specialization_blocksize_sweep.md`; results: `/home/exouser/benchkit-results/specialization-blocksize-sweep-skyler-h100-b08-<policy>-20260907`. |
| `EBLC-SPEC` post-fix gate | `specialization-vs-native-smoke-skyler-h100-b08-{off,auto}-20260906`, followed by the paired memory smoke | `js2-h100` | 2026-09-06 23:20 UTC | 315 per vs-native arm; 96 per memory arm | `COMPLETE-GATE`: exact matrix coverage in all four arms, clean detached FZGPUModules worktree at `b08c406`, and the expected CESM-2D/PSL PFPL refusal is accepted under both Off/Auto. Verification caveats are undeclared validity/timing exclusions, not missing cells, provenance, artifacts, or unexpected failures. Results: `/home/exouser/benchkit-results/specialization-{vs-native,memory}-smoke-skyler-h100-b08-<policy>-20260906`. |
